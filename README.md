...
זהו — פקודה אחת. `heb-date-core` נמשכת אוטומטית, אין תלויות ריצה נוספות ואין CSS נפרד לייבא.
...
| `monthChange` | `HebMonthRef` — `{ year, month }` | כשהחודש המוצג משתנה. מספור חודשים: ניסן=1 … אלול=6 … תשרי=7 … אדר ב׳=13 |
...
| תלות | מאיפה | תפקיד |
|---|---|---|
| `heb-date-core` | נמשכת אוטומטית | ליבת הלוח העברי (חבילת האחות) — מנוע חישוב עצמאי, ללא תלויות ריצה |
...
חישובי הלוח (המרות, ג׳מטריה, חגים) ממומשים בתוך `heb-date-core` עצמה ומאומתים
בבדיקות מול מנוע ה-ICU המובנה בדפדפן (`Intl`) על פני מאות שנים — אין אף תלות
ריצה חיצונית, והמשקל הכולל קטן בהתאם (הרכיב + הליבה ≈ ‎~15KB gzip).

## פיתוח (monorepo)

| פרויקט | תיאור |
|---|---|
| [`projects/heb-date-core`](projects/heb-date-core) | ליבת לוח עברי **אגנוסטית לפריימוורק** (TS טהור, מנוע לוח עצמאי). ללא Angular. |
...
ליבה בדיוק, והיא גם מבודדת את מנוע הלוח מאחורי קובץ אחד (`hdate-utils.ts`) —
כך הוחלף בעבר `@hebcal/core` במנוע העצמאי הנוכחי בלי לגעת באף צרכן.
...
`heb-date-core` אוטומטית (כמו ש-Angular Material מושך את `@angular/cdk`):
...
מופץ תחת **[MIT](LICENSE)** — חופשי לכל שימוש, כולל בפרויקטים קנייניים/סגורים.

עד גרסה 1.x הפרויקט השתמש ב-`@hebcal/core` לחישובי הלוח, וכיוון שרישיונה
**GPL-2.0** הוא copyleft (מחייב שכל מי שמפיץ תוכנה שמשתמשת בה יפיץ גם אותה
תחת GPL), כל הפרויקט הופץ בהתאם — מה שפסל שימוש בפרויקטים קנייניים/סגורים.
מגרסה 2.0.0 חישובי הלוח (המרות, שנים מעוברות, ג׳מטריה, שמות חודשים, חגים)
ממומשים במנוע עצמאי בתוך `heb-date-core` עצמה — **אפס תלויות ריצה** — ומאומתים
בבדיקות מול מנוע ה-ICU המובנה בדפדפן/Node (`Intl`) על כל יום לאורך מאות שנים.
זה איפשר את המעבר ל-MIT. פרטים מלאים ב-[CHANGELOG](CHANGELOG.md#200).
