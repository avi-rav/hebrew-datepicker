...
זהו. פקודה אחת — `heb-date-core` נמשכת אוטומטית. אין תלויות נוספות להתקין ידנית.
...
החודשים: ניסן=1 … אלול=6 … תשרי=7 … אדר ב׳=13).
...
| תלות | מאיפה | הערה |
|---|---|---|
| `heb-date-core` | נמשך אוטומטית | ליבת הלוח (חבילת האחות שלנו) — מנוע חישוב עצמאי, ללא תלויות ריצה |
...
ב-jQuery, Moment, או כל ספרייה כבדה נוספת: חישובי הלוח (המרות, ג׳מטריה, חגים)
ממומשים בתוך `heb-date-core` עצמה ומאומתים בבדיקות מול מנוע ה-ICU המובנה
(`Intl`) על פני מאות שנים. (משקל משוער: הרכיב + הליבה ≈ ‎~15KB gzip.)
...
מופץ תחת **MIT** — חופשי לכל שימוש, כולל בפרויקטים קנייניים/סגורים. עד גרסה 1.x
הפרויקט נשען על `@hebcal/core` (רישיון GPL-2.0, copyleft) ולכן הופץ תחתיו גם
הוא; מגרסה 2.0.0 חישובי הלוח ממומשים במנוע עצמאי בתוך `heb-date-core`
(אפס תלויות ריצה, מאומת מול `Intl`), מה שאיפשר את המעבר ל-MIT. פרטים מלאים
ב-[CHANGELOG של הפרויקט](https://github.com/avi-rav/hebrew-datepicker/blob/main/CHANGELOG.md#200).
